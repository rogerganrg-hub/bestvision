// apps/backend/scripts/audit_failure_drill.mjs
import crypto from "node:crypto";
import sqlite3 from "sqlite3";
import { request } from "node:http";

const BACKEND_BASE = process.env.BACKEND_BASE ?? "http://localhost:3000";
const DB_PATH = "D:/bestvision/apps/backend/data/bestvision.sqlite";
...
(async () => {
  console.log("NOTE: Run backend with AUDIT_FORCE_FAIL=1 before this drill.");
  console.log("BackendBase:", BACKEND_BASE);
  ...
  console.log("CHECK: Backend console should show 'audit_write_failed' for this requestId.");
})().catch((e) => {
